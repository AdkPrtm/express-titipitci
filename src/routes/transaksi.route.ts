...
router.post(
  ...
  validate(createTransaksiSchema),
...
router.get(
  ...
  authenticate,
  authorize('ADMIN', 'CASHIER'),
  cacheMiddleware({ keyPrefix: 'transaksi', ttl: 600 }),
...
router.get(
  ...
  authorize('ADMIN', 'CASHIER'),
  cacheMiddleware({ keyPrefix: 'transaksi', ttl: 600 }),
  validate(getFilterTransaksiSchema),
  transaksiController.getFilterTransaksi,
);
