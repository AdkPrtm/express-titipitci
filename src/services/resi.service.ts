...
class ResiServices {
    async createResiService(resiData: CreateResiModel): Promise<ResiResponse> {
        ...
        logger.debug(
            `Checking resi with data: ${JSON.stringify(resiData.noResi)}`,
        );
        ...
        const { id, userId, noResi, tanggalDiterima, posisiPaket, estimasiTiba, statusPaket, statusCod, cod, createdAt, updatedAt } = resi;
    ...
    async getAllResiService(): Promise<ResiResponse[]> {
        ...
        const response = resi.map(({ id, userId, noResi, tanggalDiterima, posisiPaket, estimasiTiba, statusPaket, statusCod, cod, createdAt, updatedAt }) => ({
            id,
            userId,
            noResi,
            tanggalDiterima,
    ...
    async getFilteredResiService(filter: FilterResiModel): Promise<ResiResponse[]> {
        logger.debug(`Getting filtered resi data with filter: ${JSON.stringify(filter)} `);
        const resi = await prisma.resi.findMany({
            where: filter,
            include: {
                cod: true
        ...
        if (!resi) {
            logger.error(`Failed to get filtered resi data with filter: ${JSON.stringify(filter)} `);
        ...
        logger.debug(`Filtered resi data retrieved successfully with filter: ${JSON.stringify(filter)} `);
        return response;
    }

    async updateResiService(resiData: CreateResiModel): Promise<ResiResponse> {
        logger.debug(`[${resiData.noResi}] Updating resi with data: ${JSON.stringify(resiData)}`);
        ...
        if (!resi) {
            logger.error(`Failed to update resi with data: ${JSON.stringify(resiData)}`);
        ...
        logger.debug(`Resi updated successfully with data: ${JSON.stringify(resiData)}`);
        return response;
    }
    ...
    async deleteResiService(noResi: string): Promise<ResiResponse> {
        ...
        const { deletedCod, deletedResi } = await prisma.$transaction(async (tx) => {
