...
import { FilterTransaksiModel, TransaksiRequestModel } from "../models/transaksi-request.model";
...
class TransaksiService {
     ...
     */
    async createTransaksiService(data: TransaksiRequestModel): Promise<TransaksiResponseModel> {
        logger.debug(`Creating transaksi with data: ${JSON.stringify(data)}`);

        const { user_id, nomor_resi, tanggal_diambil, metode_pembayaran, alamat_pengambilan, catatan } = data;

        const isUserExists = await prisma.user.findUnique({ where: { id: user_id } });
        ...
        const statusCod: boolean[] = isResiExists ? isResiExists.map((resi) => resi.statusCod === true) : [];
        const feeCod: number = statusCod.filter((status) => status).length * 5000;

        const { transaksi, updatedResults } = await prisma.$transaction(async (tx) => {
        ...
        if (!transaksi || !updatedResults) {
            logger.error(`Failed to create transaksi with data: ${JSON.stringify(data)}`);
    ...
    async getAllTransaksiService(): Promise<TransaksiResponseModel[]> {
        ...
        const formattedTransactions: TransaksiResponseModel[] = transaksi.map(transaction => {
            return {
                nama_penerima: {
    ...
    async getFilterTransaksiService(filter: FilterTransaksiModel): Promise<TransaksiResponseModel[]> {
