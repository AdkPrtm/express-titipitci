...
class ResiController {
  ...
  async getFilteredResi(req: Request, res: Response, next: NextFunction) {
    ...
    if (tanggalDiterima)
      filter.tanggalDiterima = new Date(String(tanggalDiterima));
    if (posisiPaket) filter.posisiPaket = String(posisiPaket);
    if (statusPaket) filter.statusPaket = String(statusPaket) as EnumProcces;
    if (statusCod !== undefined && statusCod !== null)
      filter.statusCod = /true/i.test(String(statusCod));
    if (statusPembayaranCod)
      filter.statusPembayaranCod = String(
        statusPembayaranCod,
      ) as EnumStatusPembayaranCOD;
    if (tanggalPembayaran)
      filter.tanggalPembayaran = new Date(String(tanggalPembayaran));
